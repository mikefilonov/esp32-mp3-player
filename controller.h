...
class Mp3PlayerController : public NavigationController, public PlayerEvents {
...
protected:
  ...
  void doPrev() {
    char prevFile[256] = {0};
    if (sdFindNextFile(prevFile, sizeof(prevFile), currentFile,
                       /*backwards=*/true)) {
...
public:
  void setPairingModeActive(bool active) { pairingModeActive = active; }
  ...
  void loop() {
    ...
    if (now - lastSdCheck >= 1000 || lastSdCheck == 0) {
      lastSdCheck = now;
      // Avoid slow SPI directory access during active playback to prevent audio
      // stuttering. If the card is pulled out while playing, the decoder's next
      // read will fail, stopping the player.
      ...
      if (isSdPresent != wasSdPresent) {
        ...
        if (isSdPresent) {
          ...
          if (playerIsConnected()) {
            ...
            // Revert to Blue if BT is disconnected, UNLESS we are in pairing
            // mode (Purple)
          ...
          navigationSetLedColor(255, 255,
                                0); // Yellow indicating missing storage
        }
      }
    }
    ...
    // Disable any media/playback/volume navigation until a connection is
    // established
    if (action != ControllerAction::STOP && !playerIsConnected()) {
      Serial.println(
          "[ctrl] Action ignored — waiting for Bluetooth connection...");
      return;
    }
    ...
    case ControllerAction::NEXT:
      doNext();
      break;
    case ControllerAction::PREV:
      doPrev();
      break;
    case ControllerAction::PLAY_PAUSE:
      doPlayPause();
      break;
    ...
    case ControllerAction::STOP:
      playerStop();
      if (!playerIsConnected()) {
        navigationSetLedColor(
            0, 0, 255); // Keep it Blue when disconnected/reconnecting
      } else {
  ...
  void onUpButtonPress() override { pendingAction = ControllerAction::VOL_UP; }
